...
class Trader:
    ...
    def compute_orders_c_and_pc(self, order_depth):
        ...
        if res > trade_at:
            vol = self.position['PINA_COLADAS'] + \
                self.POSITION_LIMIT['PINA_COLADAS']
            assert (vol >= 0)
            if vol > 0:
                orders['PINA_COLADAS'].append(
                    Order('PINA_COLADAS', worst_buy['PINA_COLADAS'], -vol))
        elif res < -trade_at:
            vol = self.POSITION_LIMIT['PINA_COLADAS'] - \
                self.position['PINA_COLADAS']
            assert (vol >= 0)
            if vol > 0:
                orders['PINA_COLADAS'].append(
                    Order('PINA_COLADAS', worst_sell['PINA_COLADAS'], vol))
    ...
    def compute_orders_basket(self, order_depth):
        ...
        res_buy = mid_price['PICNIC_BASKET'] - mid_price['DIP'] * \
            4 - mid_price['BAGUETTE']*2 - mid_price['UKULELE'] - 375
        res_sell = mid_price['PICNIC_BASKET'] - mid_price['DIP'] * \
            4 - mid_price['BAGUETTE']*2 - mid_price['UKULELE'] - 375
        ...
        if res_sell > trade_at:
            vol = self.position['PICNIC_BASKET'] + \
                self.POSITION_LIMIT['PICNIC_BASKET']
            self.cont_buy_basket_unfill = 0  # no need to buy rn
            assert (vol >= 0)
            if vol > 0:
                do_bask = 1
                basket_sell_sig = 1
                orders['PICNIC_BASKET'].append(
                    Order('PICNIC_BASKET', worst_buy['PICNIC_BASKET'], -vol))
                self.cont_sell_basket_unfill += 2
                pb_neg -= vol
                # uku_pos += vol
        elif res_buy < -trade_at:
            vol = self.POSITION_LIMIT['PICNIC_BASKET'] - \
                self.position['PICNIC_BASKET']
            ...
            if vol > 0:
                ...
                basket_buy_sig = 1
                orders['PICNIC_BASKET'].append(
                    Order('PICNIC_BASKET', worst_sell['PICNIC_BASKET'], vol))
        ...
        if int(round(self.person_position['Olivia']['UKULELE'])) > 0:
            ...
            if val_ord > 0:
                orders['UKULELE'].append(
                    Order('UKULELE', worst_sell['UKULELE'], val_ord))
        if int(round(self.person_position['Olivia']['UKULELE'])) < 0:

            val_ord = -(self.POSITION_LIMIT['UKULELE'] + uku_neg)
            if val_ord < 0:
                orders['UKULELE'].append(
                    Order('UKULELE', worst_buy['UKULELE'], val_ord))
    ...
    def compute_orders_dg(self, order_depth, observations):
        ...
        if self.buy_gear:
            vol = self.POSITION_LIMIT['DIVING_GEAR'] - \
                self.position['DIVING_GEAR']
            orders['DIVING_GEAR'].append(
                Order('DIVING_GEAR', worst_sell['DIVING_GEAR'], vol))
        if self.sell_gear:
            vol = self.position['DIVING_GEAR'] + \
                self.POSITION_LIMIT['DIVING_GEAR']
            orders['DIVING_GEAR'].append(
                Order('DIVING_GEAR', worst_buy['DIVING_GEAR'], -vol))
    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        result = {'PEARLS': [], 'BANANAS': [], 'COCONUTS': [], 'PINA_COLADAS': [], 'DIVING_GEAR': [
        ], 'BERRIES': [], 'DIP': [], 'BAGUETTE': [], 'UKULELE': [], 'PICNIC_BASKET': []}
        ...
        assert abs(self.position.get('UKULELE', 0)
                   ) <= self.POSITION_LIMIT['UKULELE']
        ...
        _, bs_bananas = self.values_extract(collections.OrderedDict(
            sorted(state.order_depths['BANANAS'].sell_orders.items())))
        _, bb_bananas = self.values_extract(collections.OrderedDict(
            sorted(state.order_depths['BANANAS'].buy_orders.items(), reverse=True)), 1)
        ...
        acc_bid = {'PEARLS': pearls_lb, 'BANANAS': bananas_lb}
        # we want to sell at slightly above
        acc_ask = {'PEARLS': pearls_ub, 'BANANAS': bananas_ub}
        ...
        for product in state.market_trades.keys():
            for trade in state.market_trades[product]:
                ...
                self.person_actvalof_position[trade.seller][product] += - \
                    trade.quantity
