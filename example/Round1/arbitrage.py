...
class KelpStrategy(MarketMakingStrategy):
    def estimate_value(self, state: TradingState) -> int:
        ...
        # Define smoothing factor alpha (0 < alpha <= 1). Lower values produce smoother EMA.
        alpha = 0.5

        # Initialize or update the EMA for the midpoint.
        if not hasattr(self, 'ema_mid'):
            self.ema_mid = current_mid
        else:
            self.ema_mid = alpha * current_mid + (1 - alpha) * self.ema_mid

        return round(self.ema_mid)
...
class GiftBasketStrategy(Strat_base_class):
...
class Squid_Ink_Strategy(MarketMakingStrategy):
    ...
    def save(self):
        return jsonpickle.encode(self._Squid_Ink_data)

    def load(self, data: JSON):
        if data is not None:
            self._Squid_Ink_data = jsonpickle.decode(data)
        else:
            self._Squid_Ink_data = None

    def squid_arb_clear(self, squid_ink_position: int) -> int:
        return -squid_ink_position

    def squid_adapt_edge(self, timestamp: int,
                         curr_edge: float,
                         position: int,
                         traderObject: dict) -> float:
        if timestamp == 0:
            traderObject["curr_edge"] = 2
            return traderObject["curr_edge"]

        # if timestamp is not zero

        traderObject["volumn_history"].append(abs(position))
        if len(traderObject["volumn_history"]) > PARAMS[self.symbol]["volume_avg_timestamp"]:
            traderObject["volumn_history"].pop(0)

        if len(traderObject["volumn_history"]) < PARAMS[self.symbol]["volume_avg_timestamp"]:
            return curr_edge
        elif not traderObject["optimized"]:
            volumn_avg = np.mean(traderObject["volumn_history"])

            if volumn_avg > PARAMS[self.symbol]["volume_bar"]:
                traderObject["volumn_history"] = []
                traderObject["curr_edge"] = curr_edge + \
                    PARAMS[self.symbol]["step_size"]
                return curr_edge + PARAMS[self.symbol]["step_size"]
            elif PARAMS[self.symbol]["dec_edge_discount"] * PARAMS[self.symbol]["volumn_bar"] * (curr_edge - PARAMS[self.symbol]["step_size"]) > volumn_avg*curr_edge:
                if curr_edge - PARAMS[self.symbol]["step_size"] > PARAMS[self.symbol]["min_edge"]:
                    # clear history if edge is changed
                    traderObject["volumn_history"] = []
                    traderObject["curr_edge"] = curr_edge - \
                        PARAMS[self.symbol]["step_size"]
                    traderObject["optimized"] = True
                    return curr_edge - PARAMS[self.symbol]["step_size"]
                else:
                    traderObject["curr_edge"] = PARAMS[self.symbol]["min_edge"]
                    return PARAMS[self.symbol]["min_edge"]
        traderObject["curr_edge"] = curr_edge
        return curr_edge

    def squid_ink_implied_bid_ask(self, observation: ConversionObservation) -> (float, float):
    ...
    def squid_ink_arb_take(
        ...
        implied_bid, implied_ask = self.squid_ink_implied_bid_ask(observation)
        ...
        edge = (ask - implied_ask) * \
            PARAMS[self.symbol]["make_probability"]
        for price in sorted(list(order_depth.sell_orders.keys())):
            ...
            if price < implied_bid - edge:
                ...
                if quantity > 0:
                    orders.append(
                        Order(self.symbol, round(price), quantity))
    ...
    def squid_ink_arb_make(
        ...
        if aggressive_ask >= implied_ask + PARAMS[self.symbol]['min_edge']:
            ask = aggressive_ask
            print("AGGRESSIVE")
        ...
        if buy_quantity > 0:
            orders.append(Order(self.symbol, round(
                bid), buy_quantity))  # Buy order

        sell_quantity = position_limit + (position - sell_order_volume)
        if sell_quantity > 0:
            orders.append(Order(self.symbol, round(
                ask), -sell_quantity))  # Sell order
    ...
    def decision(self, state):
        ...
        squid_ink_position = (
            state.position["SQUID_INK"]
            if "SQUID_INK" in state.position
            else 0
        )
        self.conversions = self.squid_arb_clear(
            squid_ink_position
        )

        adap_edge = self.squid_adapt_edge(
            state.timestamp,
            self._Squid_Ink_data["curr_edge"],
            squid_ink_position,
            self._Squid_Ink_data
        )
        ...
        squid_ink_orders, buy_order_volumn, sell_order_volumn = self.squid_ink_arb_take(
            state.order_depths["SQUID_INK"],
            state.observations.conversionObservations["SQUID_INK"],
            adap_edge,
            squid_ink_position
        )
        squid_ink_make_orders, _, _ = self.squid_ink_arb_make(
            state.order_depths["SQUID_INK"],
            state.observations.conversionObservations["SQUID_INK"],
            squid_ink_position,
            adap_edge,
        ...
        self.orders.extend(squid_ink_make_orders)
        self.orders.extend(squid_ink_orders)
